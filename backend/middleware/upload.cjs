...
const fileFilter = (req, file, cb) => {
  ...
  // Check MIME type, but also allow if extension is valid (some browsers send different MIME types)
  const hasValidMimeType = allowedMimeTypes.includes(file.mimetype) || 
                           (hasValidExtension && file.mimetype.startsWith('image/')) ||
                           (hasValidExtension && file.mimetype.startsWith('application/'));
