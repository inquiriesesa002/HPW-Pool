...
const getJobs = async (req, res) => {
  try {
    ...
    // Location filters: support BOTH single-field and array-field storage
    const continentId = toIdOrString(continent);
    ...
    if (continentId || countryId || provinceId || cityId) {
      jobs = jobs.filter(job => {
        ...
        return matchesContinent && matchesCountry && matchesProvince && matchesCity;
      });
    }
