/**
 * Seed Europe Cities
 * Adds all cities for all European provinces/states
 * 
 ...
 * 
 * Usage:
 *   node backend/scripts/seedEuropeCities.cjs
 *   npm run seed:europe:cities
 */
...
async function main() {
  try {
    ...
    const europeCountries = await Country.find({ region: 'europe' }).select('_id name code');
    console.log(`Found ${europeCountries.length} European countries`);

    if (europeCountries.length === 0) {
      console.log('No European countries found. Please seed countries first.');
      process.exit(0);
    }

    const provinces = await Province.find({ region: 'europe' }).select('_id name code flagImage');
    console.log(`Provinces in Europe: ${provinces.length}`);
    ...
    for (const p of provinces) {
      const country = europeCountries.find(c => {
        const countryProvinces = statesByCountryId.get(iso2ToCountryId.get(c.code));
        if (!countryProvinces) return false;
        return countryProvinces.some(s => norm(s.name) === norm(p.name));
      });
      if (country) {
        const countryId = String(country._id);
        if (!provincesByCountry.has(countryId)) {
          provincesByCountry.set(countryId, { byName: new Map(), byCode: new Map() });
        }
        const maps = provincesByCountry.get(countryId);
        maps.byName.set(norm(p.name), p);
        if (p.code) maps.byCode.set(norm(p.code), p);
      }
    }
    ...
    const totalInDB = await City.countDocuments({ region: 'europe' });
    console.log(`Total European cities in database: ${totalInDB}`);

    console.log('Europe cities seeding completed!');
  } catch (error) {
    console.error('Error seeding Europe cities:', error);
    process.exit(1);
  }
}
