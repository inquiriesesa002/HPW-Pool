/**
 * Seed missing Cities for African provinces/states that currently have 0 cities.
 *
 ...
 *
 * City uniqueness:
 * - Upsert on (name + province + country) to avoid duplicates.
 *
 * Usage:
 *   node backend/scripts/seedMissingAfricanCities.cjs
...
async function main() {
  ...
  if (!africa) throw new Error('Africa continent not found in DB.');

  const africanCountries = await Country.find({ continent: africa._id }).select('_id name code');
  const africanCountryIds = africanCountries.map(c => c._id);
  console.log('🌍 African countries:', africanCountries.length);

  const provinces = await Province.find({ country: { $in: africanCountryIds } }).select(
    '_id name code country'
  );
  console.log('🗺️ Provinces in Africa:', provinces.length);

  const provincesWithCities = new Set(
    (await City.distinct('province', { country: { $in: africanCountryIds } })).map(String)
  );
  ...
  for (const country of africanCountries) {
    ...
    for (const st of datasetStates) {
      ...
      const province =
        bucket.byName.get(norm(stateName)) ||
        (stateCode ? bucket.byCode.get(stateCode) : null);
