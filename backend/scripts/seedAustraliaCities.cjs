/**
 * Seed Australia/Oceania Cities
 * Adds all cities for all Australia region provinces/states
 * 
 ...
 * 
 * Usage:
 *   node backend/scripts/seedAustraliaCities.cjs
 *   npm run seed:australia:cities
 */
...
async function main() {
  try {
    ...
    const australiaCountries = await Country.find({ region: 'australia' }).select('_id name code');
    ...
    const provinces = await Province.find({ region: 'australia' }).select('_id name code flagImage');
    console.log(`Provinces in Australia region: ${provinces.length}`);
    ...
    const totalInDB = await City.countDocuments({ region: 'australia' });
    console.log(`Total Australia region cities in database: ${totalInDB}`);

    console.log('Australia region cities seeding completed!');
  } catch (error) {
    console.error('Error seeding Australia region cities:', error);
    process.exit(1);
  }
}
