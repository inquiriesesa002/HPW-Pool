...
// --------------------
const load = (route) => {
  // Try Vercel path first (api/server/routes/)
  let routePath = path.join(__dirname, "server", "routes", route);
  try {
    return require(routePath);
  } catch (err) {
    // Fallback to local path (../server/routes/)
    routePath = path.join(__dirname, "..", "server", "routes", route);
    return require(routePath);
  }
};
